...
class DB:
    ...
    def addVideo(self, userid, videoid, cateid, videotitle):
        try:
            self.cu.execute("insert into uservideo (userid, videoid) values (%d, '%s')" %(userid, videoid))
            self.cu.execute("insert into video (videoid, title, snsid, score, cateid) values ('%s', '%s', '', 0, %d)" \
            %(videoid, videotitle, cateid))
    ...
    def getCategory(self):
        self.cu.execute("select * from category")
        return self.cu.fetchall()

    def getCateName(self, cateid):
        self.cu.execute("select catename from category where cateid=%d" %cateid)
        return self.cu.fetchall()[0][0]

    def getCateVideos(self, cateid):
        self.cu.execute("select v.*, u.* from video as v, user as u, uservideo as uv where v.videoid=uv.videoid and \
